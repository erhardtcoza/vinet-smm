  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (pathname === '/api/ingest' && request.method === 'POST') {
        ...
        // naive product extraction from pages
        ...
        for (const p of products) {
          await env.DB
            ...
            .bind(company_id, p.title, p.url, p.summary, p.price ?? null, JSON.stringify(p.images||[]), p.tags?.join(',')||null)
            .run();
        }
      ...
      if (pathname === '/api/plan/week' && request.method === 'POST') {
        const { company_id, week_start, platforms = ['facebook','instagram','linkedin','x'] } = await request.json();
        if (!company_id || !week_start) return bad('company_id and week_start required');
        const company = await env.DB.prepare('SELECT * FROM company WHERE id=?').bind(company_id).first();
        if (!company) return bad('company not found');
        const prods = await env.DB.prepare('SELECT * FROM product WHERE company_id=? LIMIT 20').bind(company_id).all();
        const planJson = buildWeeklyPlan(company, prods.results || [], platforms);
        const { lastInsertRowid } = await env.DB
          .prepare('INSERT INTO content_plan (company_id, week_start, platform, status, json) VALUES (?, ?, ?, ?, ?)')
        ...
        for (const post of planJson.posts) {
          await env.DB
            ...
            .bind(lastInsertRowid, post.platform, post.caption, post.hashtags?.join(' '), post.image_prompt, post.scheduled_at, 'draft')
            .run();
        }
      ...
      if (pathname === '/api/competitors' && request.method === 'POST') {
        ...
        for (const c of competitors) {
          await env.DB
            ...
            .bind(company_id, c.name||null, c.url, JSON.stringify(c.socials||{}))
            .run();
        }
      ...
      if (pathname === '/api/competitors' && request.method === 'GET') {
        ...
        const rows = await env.DB.prepare('SELECT * FROM competitor WHERE company_id=?').bind(company_id).all();
        // very light analysis placeholder
      ...
      if (pathname === '/api/seo/audit' && request.method === 'GET') {
        ...
        await env.DB
          ...
          .bind(company_id, target, audit.title, audit.h1, audit.meta_desc, audit.score, JSON.stringify(audit.issues), Math.floor(Date.now()/1000))
          .run();
        return ok(audit);
      }
      ...
      if (pathname === '/api/export/zip' && request.method === 'POST') {
        ...
        const posts = await env.DB.prepare('SELECT * FROM post WHERE plan_id=? ORDER BY scheduled_at').bind(plan_id).all();
...
function ok(json) { return cors(new Response(JSON.stringify(json), { headers: { 'Content-Type': 'application/json' } })); }
function bad(msg) { return cors(new Response(JSON.stringify({ error: msg }), { status: 400, headers: { 'Content-Type': 'application/json' } })); }
...
async function upsertCompany(DB, body) {
  ...
  const res = await DB
    ...
    .bind(name, description||null, tone||null, site_url, JSON.stringify(socials||{}), logo_url||null, JSON.stringify(colors||{}))
    .run();
...
function extractProducts(pages) {
  ...
  for (const p of pages) {
    // ultra-naive: look for product-like blocks (cards with h2/h3 and links)
...
function buildWeeklyPlan(company, products, platforms) {
  ...
  for (let d=0; d<7; d++) {
    ...
    const prod = products[d % Math.max(1, products.length)] || { title: company.name, summary: company.description };
...
function baseHashtags(name, prod) {
  const tags = ["#Vinet", "#Internet", "#Connectivity", "#Fibre", "#Wireless"]; // adjust
  if (prod?.tags?.length) tags.push(...prod.tags.slice(0,3).map(t=>`#${t.replace(/\s+/g,'')}`));
  return dedupeBy(tags, x=>x);
}
function imagePrompt(prod, company) {
  return `Minimal ad tile for ${company.name}. Headline: ${prod.title}. Colors: ${company.colors_json||'brand'}. Include logo if available.`;
}

async function analyzeCompetitors(list, env) {
  // Placeholder: just returns domains & basic meta title fetch
  const out = [];
  for (const c of list) {
    try {
    ...
    } catch { out.push({ id:c.id, url:c.url, error: 'fetch_failed' }); }
  }
  return out;
}
...
async function auditPage(url) {
  ...
  const meta_desc = (html.match(/<meta[^>]*name=["']description["'][^>]*content=["']([^"']*)["'][^>]*>/i)||[])[1] || '';
...
function toCSV(rows) {
  ...
  const lines = rows.map(r=>Object.keys(r).map(k=>`"${esc(r[k])}"`).join(','));
  return header + '\n' + lines.join('\n');
}
...
function dedupeBy(arr, keyer){
  const m = new Map();
  for (const x of arr){ const k = keyer(x); if(!m.has(k)) m.set(k,x); }
  return [...m.values()];
}
